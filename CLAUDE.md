...
On every user message that involves **planning, analysis, or implementation**:
1. Check `.claude/skills/` for a matching skill.
2. If a skill matches, read its `SKILL.md` and follow the procedure before generating a freeform answer.
3. Prefer skill-guided responses over ad-hoc answers.

Skip skill lookup for quick questions, clarifications, or conversational follow-ups.
